...
datadir=([imagepath,experimentpath,'Data - testing\']);
...
nucr=8;
framesperhr=5.7;
drugspike=114/framesperhr;
drugspike2 = 386/framesperhr;
startframe = 1;
endframe = 653;
frames=startframe:endframe;
channelnames={'Cy5_' 'CFP_' 'YFP_','mCherry_'};
...
conditionlist = {'Control','DMI','Rosi','Control+CDK4-6i','DMI+CDK4-6i','Rosi+CDK4-6i'...
...
alltraces4 = zeros(numblocks,endframe);
alltraces5 = zeros(numblocks,endframe);
...
for row = 1:numrows
for site = 1:numsites
%     if rows(row) == 2 && cols(col)==9 && sites(site)==1
%         continue
%     end
...
for i = 1:tempcellcount
    alteredlength(i) = sum(~isnan(tracedata(i,114:end,1)));
...
[traces3,badtraces3]=gate_Geminin_8_mother(tracedata,tracestats,...
    motherstats,channelGem,maxthresh,minthresh,maxpos,minpos);
...
nucthresh=75;    %threshold for nuc intensity according to nucthreshoption
...
[traces2,badtraces2]=gate_Cdk2_8_mother(tracedata,nucchannel,cytochannel,...
    tracestats,nucthreshoption,nucthresh,motherthresh,noisethresh,quiescentanalysis,removequiescent);

%%% gate miscellaneous %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
traces4=tracedata(:,:,3); %%% Area
traces5=tracedata(:,:,4); % Mass (total h2b intensity median*area);
% traces2 = tracedata(:,:,7);
% traces3 = tracedata(:,:,8);
...
badtraces = badlengths | badtraces1 | overlap;%| badtraces2;% | orphan;  
traces1=traces1(~badtraces,:);
traces2=traces2(~badtraces,:);
traces3=traces3(~badtraces,:);
traces4=traces4(~badtraces,:);
traces5=traces5(~badtraces,:);
...
if storeENDindex > length(allIDs)
    ...
    alltraces4 = [alltraces4; zeros(numblocks,endframe)];
    alltraces5 = [alltraces5; zeros(numblocks,endframe)];
...
% store information 
    ...
    alltraces4(storeSTARTindex:storeENDindex,:) = traces4;
    alltraces5(storeSTARTindex:storeENDindex,:) = traces5;
...
alltraces4(excessblocks,:) = [];
alltraces5(excessblocks,:) = [];
...
% binrange = 1:5:120;
% figure('color','white','units','normalized','outerposition',[0 0 .65 1]);
% histogram(testmitosis./framesperhr,binrange);
% title({['Mitosis Times - ',conditionlist{col}]},'FontName','Arial','FontSize',30);
...
% print(gcf,'-dtiffn',[imagepath,experimentpath,'Data Summary\no CDK2\Mitosis Hist_',conditionlist{cols(col)},'.tif'])
...
%%%%%%%%%%%%%%%%%%%%%%%%%save collected data%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
save([imagepath,experimentpath,'Data Summary\Corrected-No Start requirement\',conditionlist{cols(col)},'.mat'],'alltraces1','alltraces2','alltraces3',...
