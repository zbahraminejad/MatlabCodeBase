...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
lastmitosisframe = zeros(length(allmarkedmitosis),1);
for i = 1:length(allmarkedmitosis)
    lastmitosisframe(i)= allmarkedmitosis{i}(end);
end

% testmitosis = [allmarkedmitosis{:}];
% testmitosis(testmitosis==1)=[];
histogram(lastmitosisframe./6,20,'normalization','pdf');
title({'Last Mitosis Frame'},'FontName','Arial','FontSize',30);
set(gca,'FontName','Arial','FontSize',25);
xlabel({'Mitosis Time (hour)'},'FontName','Arial','FontSize',25);
ylabel({'Normalized Probability'},'FontName','Arial','FontSize',25);
numgated=length(alltracestats(:,1));
lastmitosisframe = zeros(numgated,1);
end
legend({'Control','DMI','Rosi'},'FontName','Arial','FontSize',20);
hold off
%%
numgated = length(allmarkedmitosis);
for i = 1:numgated
    lastmitosisframe(i)= allmarkedmitosis{i}(end);
end
