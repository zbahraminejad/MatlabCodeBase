...
datadir=([projectpath,experimentpath,'Data_LowFreq\']);
rawdir=[imagepath,experimentpath,'Raw\',shot,'\'];
maskdir=[imagepath,experimentpath,'Mask_LowFreq\',shot];
maskwrite=0;
if ~exist(maskdir,'dir') && maskwrite
    mkdir(maskdir);
end
%%% setup %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
SF=2;EF=130; %0715=230 0719=216 0905=220 0920=226 JY0612=208 SS1211=230 SS0222=149
moviebin=1;
if moviebin==1
    nucr=12;
    debrisarea=200;
elseif moviebin==2
    nucr=6;
    debrisarea=100; %IX_Micro:100, Old_Axon:50
end
jitterwindow=3*nucr; %default 10xbin1=48. I get away with 36
...
for i=1:3
    ...
    nuc_mask=blobdetector(nuc_raw,nucr,blobthreshold,debrisarea);
...
for i=firstgoodindex:totalframes
    ...
    if i==firstgoodindex
        nuc_mask=segmentdeflections(nuc_mask,nucr,0.5,debrisarea); %attempt on all non-debris
    end
    ...
    nanvec=ones(numlivecells,1)*NaN; YFP_sig=nanvec; RFP_sig=nanvec; 
    ...
    %%% subtract background %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if ringcalc==1
        YFPprctile=10; RFPprctile=50; %cytosolic:10 nuclear:50
    else
    ...
    %%% store data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if ringcalc==1
        tracedata(cellid,i,:)=[nuc_center(:,1),nuc_center(:,2),nuc_area,nuc_mass,YFP_sig,RFP_sig,YFP_sigring];
    else
        tracedata(cellid,i,:)=[nuc_center(:,1),nuc_center(:,2),nuc_area,nuc_mass,YFP_sig,RFP_sig];
    end
