...
experimentpath='Mary\MT150310-YFP-PPARGtimecourse\';
...
% datadir=[imagepath,experimentpath,'test\'];
...
debrisarea=150;
boulderarea=2500; %MCF-10A: H2B:20 NLS:10
...
if ringcalc==1
%     parameternum=13; % for two rings
    parameternum=10; % for 1 ring
...
for i=firstgoodindex:totalframes
    ...
    if i==165
        rawdir=[imagepath,experimentpath,'Raw2\',wellname,shot,'_'];
    end
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     imwrite(uint16(real3),[realdir,name3,num2str(f),'.tif']);
    ...
    if i>firstgoodindex+1 && (numcells<numthresh || mednuc>blurthreshhigh || mednuc<blurthreshlow) || ismember(f,[31 32])  
    ...
    if ringcalc==1
        ...
        bgthreshold = abs(prctile(real2(~nanmask),5)); % assume left tail is more accurate than right tail
...
%         sig3ring_75th=nanvec; sig3ring_fgmedian=nanvec; sig3ring_fgmode=nanvec;
        for n=1:numlivecells
            ...
            ring2foreground=ring2all(ring2all>bgthreshold);
%             figure,hist(ring2all,20)
...
%         tracedata(cellid,f,:)=[nuc_center(:,1),nuc_center(:,2),nuc_area,nuc_mass,sig1,sig2,sig2ring_75th,sig2ring_fgmedian,sig2ring_fgmode,sig3,sig3ring_75th,sig3ring_fgmedian,sig3ring_fgmode];
        tracedata(cellid,f,:)=[nuc_center(:,1),nuc_center(:,2),nuc_area,nuc_mass,sig1,sig2,sig2ring_75th,sig2ring_fgmedian,sig2ring_fgmode,sig3];
    else 
