...
SF=1;EF=198; %Sabrina20x:208 Steve20x:218 Steve10x:110 Steve&Sabrina:240
...
REname = '_DHB_';
CEname = '_FRET_';  %Pilot:'cdt1' Steve20x:'geminin' Steve10x:'geminin' Steve&Sabrina:'cdt1'

debrisarea=pi*(nucr/3)^2;
...
for f=SF:EF
    ...
    %%% image processing %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [DAs_bs,~]=bgsub_MC_block(log(DAs_or),blockheight,blockwidth);
    DAs_bl=imfilter(DAs_or,fspecial('disk',floor(nucr/4)),'symmetric');
    DAs_bs_abs=bgsub_MC_block(DAs_bl,blockheight,blockwidth);
    [REs_bs,~]=bgsub_MC_block(log(REs_or),blockheight,blockwidth);
    
    %%% nuclear segmentation %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    DAs_pad=getnucmask_histsweep_test(DAs_bs,nucr);  %MC histogram sweep & concave detector
    CEs_bl=imfilter(CEs_or,fspecial('disk',floor(nucr/4)),'symmetric');
    CEs_bs=bgsub_MC_mask(CEs_bl,DAs_pad,blockheight,blockwidth);
    FRET_ratio=CEs_bs./DAs_bs_abs;
    FRET_ratio(DAs_pad==0)=0;
    FRET_ratio(FRET_ratio<0)=0;
    ...
    for cc=1:numcells
        ...
        FRETdata(cc,1)=median(FRET_ratio(DAs_da(cc).PixelIdxList));
        
        RR(cc,1)=median(REs_bs(DAs_da(cc).PixelIdxList));
        ring(cc,1)=median(REs_bs(fcr_da(cc).PixelIdxList));
        
    end
    wellsss{f-initF+1}=[XX,YY,DD,AC,RR,FRETdata,ring]; %matrix.  rows are cells.  columns are x coor, y coord, nuclear intensity, area of nucleus, reporter intensity. for this frame
    toc(timeframe)
end
save([datadir,'wellsss_', shot, '_filt.mat'],'wellsss');
cd ..\Processing; %return to this directory
toc(timetotal);
%{
%%% debugging: view images %%%%%%%%%%
tempframe=imadjust(mat2gray(FRET_ratio));
