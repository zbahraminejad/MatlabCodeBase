...
addpath(genpath('H:\Documents\MATLAB\Imaging'));
%addpath('C:\Program Files\MATLAB\R2012b\toolbox\images\images\private');
...
for i=1:totalframes
    ...
    negmask=bwareaopen(nuc_mask,2*height+2*width);
    nuc_mask=logical(nuc_mask-negmask);
    %nuc_info=struct2cell(regionprops(nuc_mask,'Centroid','Solidity')');
    %centers=squeeze(cell2mat(nuc_info(1,:,:)))';
    %solidity=squeeze(cell2mat(nuc_info(2,:,:)));
    %winrad=nucr*3; panelsperside=5;
    %gated = find(solidity<0.9);
    %centers=centers(gated,:);
    %extractmask=bwmorph(nuc_mask,'remove');
    %makepanel(nuc_raw,extractmask,centers,winrad,panelsperside);
    
    
    nuc_mask=segmentdeflections_test(nuc_mask,nucr,0.5); %third arg: min cell size ratio for calc
    ...
    %%% store data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    wellsss{i}=[XX,YY,nuc_intensity,nuc_area,YFPnuc,RFPnuc,YFPring];
