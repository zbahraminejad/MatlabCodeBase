...
for i=firstgoodindex:totalframes
    ...
    nuc_mask=blobdetector(nuc_raw,nucr);
    if i==firstgoodindex
        nuc_mask=segmentdeflections(nuc_mask,nucr,3); %0.5
    end
    %%% extract features %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if i==firstgoodindex
        ...
        %[jitx,jity]=getjitter_mass(nuc_center,nuc_mass,tracedata{lastgoodframe}(:,1:3),dims);
        centersprev=tracedata{lastgoodframe}(:,[1 2]);
        areasprev=tracedata{lastgoodframe}(:,3);
        [jitx,jity]=getjitter(nuc_center,centersprev,nuc_area,areasprev,dims);
        ...
        initdata=[nuc_center(:,1),nuc_center(:,2),nuc_mass];
        ...
        [tracked,newdaughters,nuc_label]=adaptivetrack_debug(tracedata{lastgoodframe}(:,1:3),initdata,nuc_raw,nuc_label,nucr,extractmask,jitx,jity);
        daughters=[daughters;newdaughters];
        numcells=size(tracked,1);
        nuc_center=tracked{i}(:,[1 2]);
        nuc_area=tracked{i}(:,3);
        nuc_mass=tracked{i}(:,4);
        badframes(i)=0;
    end
