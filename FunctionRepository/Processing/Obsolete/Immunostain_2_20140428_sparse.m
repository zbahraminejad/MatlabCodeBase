...
datadir = ([projectpath,experimentpath,'Data_ConstantBG_50th_min\']);
...
% iqrmult=5; %was 4 before 2014/4/17
...
%bg1=blocksmooth_mode(blur1,1);
%bg2=blocksmooth(blur2,10);
%bg2=blocksmooth_mode(blur2,1);
%bg2=ones(size(blur2))*prctile(blur2(:),10);
%bg3=blocksmooth(blur3,10);
%bg3=blocksmooth_mode(blur3,1);
%bg4=blocksmooth(blur4,10);
%bg4=blocksmooth_mode(blur4,1);
%bg1=blocksmooth_mode(blur1,1);
%%% 46Hys-under-2i
%c1=920; c2=66; c3=166; c4=175;
%%% DHBhys
%c1=450; c2=220; c3=170; c4=17;
c1=min(blur1(:)); c2=min(blur2(:)); c3=min(blur3(:)); c4=min(blur4(:));
...
for cc=1:numcells
    ...
    ringall=real2(ring_info(cc).PixelIdxList);
     %sig2_ring(cc)=median(ringall(ringall>=median(ringall)));
     %sig2_ring(cc)=median(ringall(ringall>=prctile(ringall,25)));
     sig2_ring(cc)=median(ringall);
...
IFdata=[nuc_center(:,1),nuc_center(:,2),nuc_area,sig1,sig2,sig2_ring,sig3,sig4];
