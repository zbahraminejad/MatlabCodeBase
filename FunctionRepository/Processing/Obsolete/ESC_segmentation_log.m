...
for f=SF:EF
    ...
    DAs_bs=bgsub(log(DAs_or),nucr,0.3);
    
    sigma=nucr/sqrt(2);
    h=sigma^2*fspecial('log',[40 40],sigma);
    DAs_log=imfilter(DAs_bs,h,'symmetric');
    logmask=DAs_log>0.2;  %ID background in colonies

    %%% image processing %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    totalbg=ThreshImage_MC_block(DAs_bs);
    DAs_bs(logmask==1)=totalbg;
...
tempframe(:,:,3)=0;
imshow(tempframe);
imwrite(tempframe,[path,'LoG.tif']);
%}