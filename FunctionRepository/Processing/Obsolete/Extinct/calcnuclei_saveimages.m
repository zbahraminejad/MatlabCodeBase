...
rawdir = ([path,'Raw\']);
cpdir = [path,'CroppedProcessedEval\'];
SF=1;EF=3; %208
...
for row=3
    for col=1
        ...
        M=avifile([datadir,movieName, '.avi'],'compression','none','fps',4);
        ...
        for f=SF:EF
            ...
            REfile=[movieName,'_EYFP_',num2str(f),'.tif'];
            CEfile=[movieName,'_ECFP_',num2str(f),'.tif'];
            ...
            imwrite(nuclearedges,[cpdir,DAfile]);
            imwrite(uint16(REs_bs),[cpdir,REfile]);
            imwrite(uint16(CEs_bs),[cpdir,CEfile]);            
        ...
        %%
        save([datadir,'wellsss_', movieName, '.mat'],'wellsss')
        M=close(M);
        end
    end
end
