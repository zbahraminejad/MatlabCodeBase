...
for row=4 %should be 3:4
    for col=1
        ...
        parfor f=SF:EF
            ...
            %tempframe=zeros(size(DAs_bs,1),size(DAs_bs,2),3);
            ...
            %%
            [XX,YY,DD,AC,RR,CCC,avgringyfp] = FeatureCount_parallel(DAs_da, DAs_bs, REs_bs, CEs_bs, ringxypos);
%             mz = size(DAs_da,1);
%             
...
%         %%  use this if you don't need to filter the data 
        ...
        for i=SF:EF
            M=addframe(M,im2frame(tempmovie{i}));
        end
        M=close(M);
        end
    end
