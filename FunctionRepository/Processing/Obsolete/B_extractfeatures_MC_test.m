...
for f=SF:EF
    ...
    [DAs_da,realnuc_la,finalcytoring,seg_range,seg_normrange]=buildcytoring_segrange_outer2(DAs_pad,REs_bs,nucr);
    ...
    DAs_da(nucexclude)=[];
    fcr_da(nucexclude)=[];
    seg_range(nucexclude)=[];
    seg_normrange(nucexclude)=[];
    numcells=size(DAs_da,1);
    ...
    for cc=1:numcells
        ...
        fifth=prctile(REs_bs(DAs_da(cc).PixelIdxList),5);
        ninetyfifth=prctile(REs_bs(DAs_da(cc).PixelIdxList),95);
        RRrange(cc,1)=ninetyfifth-fifth;
        %RRrange(cc,1)=range(REs_bs(DAs_da(cc).PixelIdxList));
        fifth=prctile(REs_bs(fcr_da(cc).PixelIdxList),5);
        ninetyfifth=prctile(REs_bs(fcr_da(cc).PixelIdxList),95);
        ringrange(cc,1)=ninetyfifth-fifth;
        %ringrange(cc,1)=range(REs_bs(fcr_da(cc).PixelIdxList));
        
    end
    wellsss{f-initF+1}=[XX,YY,DD,AC,RR,CCC,ring,RRrange,ringrange,seg_range,seg_normrange]; %matrix.  rows are cells.  columns are x coor, y coord, nuclear intensity, area of nucleus, reporter intensity. for this frame
    toc(timeframe)
end
save([datadir,'wellsss_', shot, '_HS_L05_595OfSegs_outer2.mat'],'wellsss');
