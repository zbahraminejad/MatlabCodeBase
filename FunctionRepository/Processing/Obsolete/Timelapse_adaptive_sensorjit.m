...
for i=firstgoodindex:totalframes
    ...
    YFP_info=struct2cell(regionprops(nuc_mask,YFP_raw,'Area','Centroid','MeanIntensity')');
    YFP_density=squeeze(cell2mat(YFP_info(3,1,:)));
    %%% subtract background %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %nuc_bg=getbackground_H2B(nuc_mask,nuc_center,nuc_raw,nucr,0.25);
    [nuc_bg,YFP_bg]=getbackground_YR(nuc_mask,nuc_center,nuc_raw,YFP_raw,nucr,0.25);
    nuc_density=nuc_density-nuc_bg;
    YFP_density=YFP_density-YFP_bg;
    %%% calculate masses %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if i==firstgoodindex
        ...
        [jitx,jity]=getjitter_loners(nuc_center,tracedata{lastgoodframe}(:,1:2),dims);
        ...
        [tracked,newdaughters,nuc_label]=adaptivetrack_concatenate(tracedata{lastgoodframe}(:,1:4),initdata,nuc_raw,nuc_label,nucr,extractmask,jitx,jity,f);
    ...
    for cc=cellid'
        YFPnuc(cc)=mean(YFP_raw(nuc_info(cc).PixelIdxList));
