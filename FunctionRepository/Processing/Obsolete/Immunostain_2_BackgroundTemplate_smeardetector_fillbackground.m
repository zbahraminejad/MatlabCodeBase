...
raw2=raw2-bgtemplate2; %raw2s=raw2; raw2s(raw2s<1)=1; raw2s=log(imfilter(raw2s,fspecial('disk',6),'symmetric'));
raw3=raw3-bgtemplate3; %raw3s=raw3; raw3s(raw3s<1)=1; raw3s=log(imfilter(raw3s,fspecial('disk',6),'symmetric'));
raw4=raw4-bgtemplate4; %raw4s=raw4; raw4s(raw4s<1)=1; raw4s=log(imfilter(raw4s,fspecial('disk',6),'symmetric'));
if separatedirectories
...
nuc_mask_thickened=bwmorph(nuc_mask,'thicken',2);
...
nuc_centers=squeeze(cell2mat(struct2cell(regionprops(nuc_label,'Centroid')')))';
nuc_pixels=regionprops(nuc_label,'PixelIdxList');
%%% detect smears %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
smearrad=10;
...
raw1withoutcells=raw1;
raw2withoutcells=raw2;
raw3withoutcells=raw3;
raw4withoutcells=raw3;
bg1=centroidbackground(nuc_mask_thickened,nuc_centers,raw1,nucr,2,0.25,50);
bg2=centroidbackground(nuc_mask_thickened,nuc_centers,raw2,nucr,2,0.25,50);
bg3=centroidbackground(nuc_mask_thickened,nuc_centers,raw3,nucr,2,0.25,50);
bg4=centroidbackground(nuc_mask_thickened,nuc_centers,raw4,nucr,2,0.25,50);
for i=1:numcells
    raw1withoutcells(nuc_pixels(i).PixelIdxList)=bg1(i);
    raw2withoutcells(nuc_pixels(i).PixelIdxList)=bg2(i);
    raw3withoutcells(nuc_pixels(i).PixelIdxList)=bg3(i);
    raw4withoutcells(nuc_pixels(i).PixelIdxList)=bg4(i);
end
raw1withoutcells(raw1withoutcells<1)=1; %raw1withoutcells=log(imfilter(raw1withoutcells,fspecial('disk',nucr/2),'symmetric'));
raw2withoutcells(raw2withoutcells<1)=1; %raw2withoutcells=log(imfilter(raw2withoutcells,fspecial('disk',nucr/2),'symmetric'));
raw3withoutcells(raw3withoutcells<1)=1; %raw3withoutcells=log(imfilter(raw3withoutcells,fspecial('disk',nucr/2),'symmetric'));
raw4withoutcells(raw4withoutcells<1)=1; %raw4withoutcells=log(imfilter(raw4withoutcells,fspecial('disk',nucr/2),'symmetric'));
raw1withoutcells=imresize(raw1withoutcells,0.1,'bicubic');
...
%[smcen3,smrad3]=imfindcircles(raw3withoutcells,[6 13]);
%smear1=smeardetector(raw1withoutcells,smearrad,smearthreshold,smeararea);
