...
%%% setup %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
SF=1;EF=47; %0715=230 0719=216 0905=220 0920=226 JY0612=208 SS1211=230 SS0222=149 20140208_p21dCy1dK=229
moviebin=1;
if moviebin==1
    %nucr=12;
...
tracedata=ones(maxcellnum,totalframes,7+ringcalc)*NaN; %default 7
...
for i=firstgoodindex:totalframes
    ...
    if ringcalc==1
        ...
        sig2ring_50th=nanvec; sig2ring_0th=nanvec;
        for n=1:numlivecells
            cc=cellid(n);
            ringall=real2(ring_info(cc).PixelIdxList);
             sig2ring_50th(n)=median(ringall(ringall>=median(ringall))); %previously mean
             sig2ring_0th(n)=median(ringall);
        end
    end
    %%% store data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if ringcalc==1
        %tracedata(cellid,i,:)=[nuc_center(:,1),nuc_center(:,2),nuc_area,nuc_mass,sig1,sig2,sig3,sig2ring];
        tracedata(cellid,i,:)=[nuc_center(:,1),nuc_center(:,2),nuc_area,nuc_mass,sig1,sig2,sig2ring_50th,sig2ring_0th];
    ...
    if maxcellnum-max(cellid)<3000
        tempdata=ones(blocksize,totalframes,7+ringcalc)*NaN; %default 7
