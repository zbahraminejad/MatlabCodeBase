function Timelapse_merge(row,col,site)
row=2;col=2;site=1;
%%% set paths %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%% setup %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
SF=1;EF=103; %0715=230 0719=216 0905=220 0920=226 JY0612=208 SS1211=230 SS0222=149
moviebin=1;
if moviebin==1
    nucr=12;
...
for i=firstgoodindex:totalframes
    ...
    %fprintf('numcells=%0.0f numthresh=%0.0f mednuc=%0.0f blurthresh=%0.0f\n',numcells,numthresh,mednuc,blurthresh);
    ...
    %%% subtract background %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    nuc_bg=centroidbackground(nuc_mask,nuc_center,raw1,nucr,10,0.25,50);
    ...
    if i>firstgoodindex
        ...
        [tracedata,curdata,tracking,nuc_label]=adaptivetrack_merge(lastgoodframe,f,tracedata,curdata,tracking,raw1,nuc_label,nucr,jitters(i,:));
        badframes(i)=0;
    end
    ...
    nanvec=ones(numlivecells,1)*NaN; sig2=nanvec; sig3=nanvec; 
    for n=1:numlivecells
        cc=cellid(n);
        sig2(n)=median(raw2(nuc_info(cc).PixelIdxList));
        sig3(n)=median(raw3(nuc_info(cc).PixelIdxList));
    end
    if ringcalc==1
        ...
        for n=1:numlivecells
            cc=cellid(n);
            ringall=raw2(ring_info(cc).PixelIdxList);
             sig2ring(n)=mean(ringall(ringall>=median(ringall)));
        end
    end
    %%% subtract background %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    bg2=centroidbackground(nuc_mask,nuc_center,raw2,nucr,20,0.25,10); %Heewon Cy5:25%
    bg3=centroidbackground(nuc_mask,nuc_center,raw3,nucr,10,0.25,50); %Heewon FRET:50%
    sig2=sig2-bg2;
    sig3=sig3-bg3;
    if ringcalc==1
        sig2ring=sig2ring-bg2;
    end
    %%% store data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if ringcalc==1
