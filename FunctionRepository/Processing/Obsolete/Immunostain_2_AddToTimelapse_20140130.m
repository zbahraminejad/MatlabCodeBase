...
%%% load images %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
raw1=single(imread([rawdir,name1,'poststain.tif']));
raw2=single(imread([rawdir,name2,'poststain.tif']));
raw3=single(imread([rawdir,name3,'poststain.tif']));
IFraw1=single(imread([rawdir,IFname1,'poststain.tif']));
...
for cc=cellid'
    ...
    IFsig1(cc)=max(IFraw1(nuc_info(cc).PixelIdxList)); %use max for EdU
    ...
    IFsig2(cc)=mean(IFraw2(nuc_info(cc).PixelIdxList)); %use mean for Hoechst
end
%%% subtract background %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
nuc_center_org=nuc_center(cellid,:)-ones(numel(cellid),1)*IFjitter;
bg2=centroidbackground(nuc_mask_withborders,nuc_center_org,raw2,nucr,10,0.25,10);
bg3=centroidbackground(nuc_mask_withborders,nuc_center_org,raw3,nucr,3,0.25,50);
IFbg1=centroidbackground(nuc_mask_withborders,nuc_center_org,IFraw1,nucr,3,0.25,50);
IFbg2=centroidbackground(nuc_mask_withborders,nuc_center_org,IFraw2,nucr,3,0.25,50);
...
save([datadir,'IF20perc_',shot,'.mat'],'IFdata','IFjitter');
toc(timetotal);
%{
