...
% experimentpath='11202014-Michael-CellCycdle-48hour-glass\';
experimentpath='12152014-Marys-Exp\'; %'12072014-Michael-48hr-CC\';
...
debrisarea=23;
boulderarea=500; %MCF-10A: H2B:20 NLS:10
...
bgcmos = imresize(bgcmos,0.5);
load([biasdir,name1,'Stain_',num2str(site),'.mat']); bias1=bias;
...
for i=firstgoodindex:totalframes
    ...
    timeframe=tic;
    %%% read images %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    raw1=double(imread([rawdir,name1,num2str(f),'.tif'])); raw1=(raw1-bgcmos)./bias1;
    raw1 = imadjust(mat2gray(raw1));
    raw2=double(imread([rawdir,name2,num2str(f),'.tif'])); raw2=(raw2-bgcmos)./bias2;
    raw3=double(imread([rawdir,name3,num2str(f),'.tif'])); raw3=(raw3-bgcmos)./bias3;
    %%% segment nuclei %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    if i==firstgoodindex
        %%% LoG filter %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%         nuc_mask = blobdetector_3(log(raw1),nucr,blobthreshold,debrisarea);
        nuc_mask = blobdetector_3_mary(raw1,nucr,blobthreshold,debrisarea);
        foreground = nuc_mask;
...
%     imwrite(uint16(real2),[realdir,name3,num2str(f),'.tif']);
    %%% extract features %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if i>firstgoodindex+1 && (numcells<numthresh || mednuc>blurthreshhigh || mednuc<blurthreshlow)  
        ...
        if numcells<numthresh
            fprintf('lost too many cells');
        elseif mednuc>blurthreshhigh
            fprintf('nuc to large');
        elseif mednuc<blurthreshlow
            fprintf('nuc to small');
        end
        badframes(f)=1;
