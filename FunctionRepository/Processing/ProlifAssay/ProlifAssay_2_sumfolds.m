...
tdiff=t1-t0;
tdiffnorm1=(tdiff./t0)+1;    %normalized difference

load([path,'cellcountmatrix_24hr.mat']);
t0=sum(cellcountmatrix,3);
load([path,'cellcountmatrix_48hr.mat']);
t1=sum(cellcountmatrix,3);
tdiff=t1-t0;
tdiffnorm2=(tdiff./t0)+1;    %normalized difference

tdiffnorm=tdiffnorm1+tdiffnorm2;

controlfig=figure;
set(controlfig,'color','w','PaperPosition',[0 0 3 3]); %cumulative stairs (3x3)
subaxis(1,1,1,'ML',0.2,'MR',0.2','MT',0.1,'MB',0.1);
controlvals=tdiffnorm1(1,[1 3 5])+tdiffnorm2(1,[1 3 5]);
