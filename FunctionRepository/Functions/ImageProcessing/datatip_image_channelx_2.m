...
winrad=4*nucr;    %window radius: default 5
imagescale=4;     %resize image so it's bigger
% framesperhr=5;
%%% setup %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
imagefignum=plotfignum.Number+1;
...
[subh,subw] = size(cropraw);
% finalmask = zeros(subh*imagescale,subw*imagescale,3);
%%% calc background %%%
...
% finalmask(:,:,1) = 0;
% finalmask(:,:,2) = 0;
% finalmask(:,:,3) = mat2gray(imresize(cropraw,imagescale),[0 400]);
% image(finalmask);
