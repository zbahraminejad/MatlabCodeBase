function output_txt = datatip_image_singletrace_cdt(obj,event_obj)
...
%rawimage2 = single(imread([path,'CroppedProcessed\',moviename,'_geminin_',num2str(frame),'.tif']));
%rawimage = single(imread([path,'Processed\',moviename,'\nucedge&ring_',framestring,'.tif']));
%rawimage2 = single(imread([path,'Raw\',moviename,'\TexasRed_',framestring,'.tif']));
rawimage = single(imread([maskdir,'nucedge_',framestring,'.tif']));
rawimage2 = single(imread([rawdir,'\TexasRed_',framestring,'.tif']));
