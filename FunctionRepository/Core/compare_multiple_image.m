% Up to six images. Write something more general if you want to see more.
% e.g., comptwo(I1, I2, I3, I4) or
% e.g., comptwo(I) where I has multiple third dimensions

function compare_multiple_image(varargin)

    images = varargin;
    % For third dimension which shouldn't exceed six (6)
    if size(images{1},3) > 1
       for i = 1:size(images{1},3)
           tempImage{i} = images{1}(:,:,i);
       end
       images = tempImage;
    end
    ...
    if numel(images) < 4 && numel(images) > 1
        for i = 1:nFig
            subaxis(1,nFig, i,'M',0.005,'S',0.01);
            imshow(images{i},[]);        
            set(gca, 'XTick', [],'YTick',[]);
            h(i) = gca;
        end
    elseif numel(images) >= 4 && numel(images) < 7
        for i = 1:nFig
            subaxis(2,3, i,'M',0.005,'S',0.01);
            imshow(images{i},[]);        
            set(gca, 'XTick', [],'YTick',[]);
            h(i) = gca;
        end
...
% Create axes in tiled positions. (just like subplot)
%   Usage:
...
% 2001-2014 / Aslak Grinsted  (Feel free to modify this code.)
    ...
    if isstruct(Args)
        UserDataArgsOK = ...
            isfield(Args,'SpacingHorizontal') & ...
            isfield(Args,'Holdaxis') & ...
            isfield(Args,'rows') & ...
            isfield(Args,'cols');
    end
    ...
    switch length(Args.NumericArguments)
       case 0
           return % No arguments but rows/columns
       case 1
          if numel(Args.NumericArguments{1}) > 1
              % Restore subplot(m,n,[x y]) behaviour
              % subplot and ind2sub count differently 
              % (column instead of row first) --> switch cols/rows
              [x1 y1] = ind2sub([Args.cols Args.rows],Args.NumericArguments{1}(1));
